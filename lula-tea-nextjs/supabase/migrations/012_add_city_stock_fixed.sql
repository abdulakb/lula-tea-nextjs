...
DECLARE
  ...
  v_new_stock INTEGER;
  v_movement_id UUID;
  v_new_riyadh_stock INTEGER;
  v_new_jeddah_stock INTEGER;
...
BEGIN
  ...
  -- Calculate new city stocks and total
  IF p_city = 'Riyadh' THEN
    v_new_riyadh_stock := v_new_stock;
    v_new_jeddah_stock := v_product.jeddah_stock;
  ELSIF p_city = 'Jeddah' THEN
    v_new_riyadh_stock := v_product.riyadh_stock;
    v_new_jeddah_stock := v_new_stock;
  END IF;
  
  v_new_total_stock := v_new_riyadh_stock + v_new_jeddah_stock;

  -- Update product stock for the specific city and total stock
  UPDATE products
  SET 
    riyadh_stock = v_new_riyadh_stock,
    jeddah_stock = v_new_jeddah_stock,
    stock_quantity = v_new_total_stock,
    available = CASE 
      WHEN v_new_total_stock <= 0 AND NOT allow_backorder THEN false
      ELSE available
    END,
